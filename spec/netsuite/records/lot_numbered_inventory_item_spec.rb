...
describe NetSuite::Records::LotNumberedInventoryItem do
  let(:item) { NetSuite::Records::LotNumberedInventoryItem.new }
  ...
  describe '#locations_list' do
    ...
    it 'can be set from an InventoryItemLocationsList object' do
      expect(NetSuite::Actions::Get).to receive(:call)
        .with([NetSuite::Records::LotNumberedInventoryItem, :internal_id => 20], {})
        .and_return(response)
      item = NetSuite::Records::LotNumberedInventoryItem.get(20)
      expect(item).to be_kind_of(NetSuite::Records::LotNumberedInventoryItem)
  ...
  describe '.get' do
    context 'when the response is successful' do
      ...
      it 'returns a LotNumberedInventoryItem instance populated with the data from the response object' do
        expect(NetSuite::Actions::Get).to receive(:call).with([NetSuite::Records::LotNumberedInventoryItem, {:external_id => 1}], {}).and_return(response)
        item = NetSuite::Records::LotNumberedInventoryItem.get(:external_id => 1)
        expect(item).to be_kind_of(NetSuite::Records::LotNumberedInventoryItem)
    ...
    context 'when the response is unsuccessful' do
      ...
      it 'raises a RecordNotFound exception' do
        expect(NetSuite::Actions::Get).to receive(:call).with([NetSuite::Records::LotNumberedInventoryItem, {:external_id => 1}], {}).and_return(response)
        expect {
          NetSuite::Records::LotNumberedInventoryItem.get(:external_id => 1)
        }.to raise_error(NetSuite::RecordNotFound,
          /NetSuite::Records::LotNumberedInventoryItem with OPTIONS=(.*) could not be found/)
      end
    end
  end

  describe '#add' do
    let(:item) { NetSuite::Records::LotNumberedInventoryItem.new(:cost => 100, :is_inactive => false) }
  ...
  describe '#record_type' do
    it 'returns a string representation of the SOAP type' do
      expect(item.record_type).to eql('listAcct:LotNumberedInventoryItem')
    end
  end
  ...
  describe '.update_list' do
        ...
        savon.expects(:update_list).with(:message =>
          {
            'record' => [{
              ...
              '@xsi:type' => 'listAcct:LotNumberedInventoryItem',
              '@internalId' => '624113'
            }]
        }).returns(File.read('spec/support/fixtures/update_list/update_list_one_item.xml'))
      end

      it 'returns collection with one LotNumberedInventoryItem instances populated with the data from the response object' do
        items = NetSuite::Records::LotNumberedInventoryItem.update_list([
                  NetSuite::Records::LotNumberedInventoryItem.new(internal_id: '624113', item_id: 'Target', upccode: 'Target')
                ])
        shutter_fly = items[0]
        expect(shutter_fly).to be_kind_of(NetSuite::Records::LotNumberedInventoryItem)
    ...
    context 'with two items' do
      ...
      it 'returns collection of LotNumberedInventoryItem instances populated with the data from the response object' do
        items = NetSuite::Records::LotNumberedInventoryItem.update_list( [
                  NetSuite::Records::LotNumberedInventoryItem.new(internal_id: '624172', item_id: 'Shutter Fly', upccode: 'Shutter Fly, Inc.'),
                  NetSuite::Records::LotNumberedInventoryItem.new(internal_id: '624113', item_id: 'Target', upccode: 'Target')
                ])
        shutter_fly = items[0]
        expect(shutter_fly).to be_kind_of(NetSuite::Records::LotNumberedInventoryItem)
